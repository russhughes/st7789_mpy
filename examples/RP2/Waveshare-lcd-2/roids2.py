...
roid.py - Asteroids style game demo using polygons.
    Raspberry Pi PICO with a Waveshare Pico LCD 2
    Display Module.
...
def main():
    ...
    try:
        ...
        tft = st7789.ST7789(
            ...
            rotation=3,
            buffer_size=font.WIDTH * font.HEIGHT * 2)
        ...
        left = Pin(2, Pin.IN, Pin.PULL_UP)      # Top Left
        right = Pin(3, Pin.IN, Pin.PULL_UP)     # Top Right
        fire = Pin(15, Pin.IN, Pin.PULL_UP)     # Bottom Left
        thrust = Pin(17, Pin.IN, Pin.PULL_UP)   # Bottom Right

        # target frame rate delay
        frame_time = 60

        label_top = font.HEIGHT
        label_bottom = height - font.HEIGHT - 1
        label_right = width - font.WIDTH*2 - 1
        label_left = font.WIDTH

        # game loop
        while True:
            last_frame = utime.ticks_ms()

            tft.text(font, "<", label_left, label_top)
            tft.text(font, ">", label_right, label_top)
            tft.text(font, "T", label_left, label_bottom)
            tft.text(font, "F", label_right, label_bottom)
